  ...
  <div class="terminal-output" ref="terminalOutput">
    <div class="show-previous-commands" id="prev-commands">
      <div v-for="(command, i) in storeCommand" :key="i">
        ...
        <div v-else-if="command === 'resume'">
          <p> Opening the github resume link...</p>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      currentTab: '',
      storeCommand: [],
      correctCommands: ["help", "projects", "about", "experience", "contact", "clear", "education", "skills", "repo", "resume"],
      currentIndex: -1,
      previousCommands: [],
      predictedCommand: '',
    };
  ...
  methods: {
    ...
    displayCommandOutput() {
      if (this.inputValue === "clear") {
        ...
        window.open("https://github.com/pj323/", "_blank");
      } else if (this.inputValue.toLowerCase().trim() === "resume") {
        const command = this.inputValue.toLowerCase().trim();
        this.addToCommandHistory(command);
        window.open("https://github.com/pj323/pj323/blob/main/Preetham%20John%20Resume.pdf", "_blank");
      } 
      else {
        const command = this.inputValue.toLowerCase().trim();
        this.addToCommandHistory(command);
